...
data = data[['Hardness, Vickers', 'Elongation at Break', 'Thermal Conductivity']]

features = ['Hardness, Vickers', 'Thermal Conductivity']
target = ['Elongation at Break']
...
ind = np.nonzero(X[:, 1] >= 1)[0]
axes2.scatter(X[:, 0], X[:, 1], y, depthshade = False, color='r')
ind = np.nonzero((X[:, 1] < 1) * (X[:, 1] > -0.2))[0]
axes2.scatter(X[ind, 0], X[ind, 1], y[ind], depthshade = False, color='g')
ind = np.nonzero(X[:, 1] <= -0.2)[0]
axes2.scatter(X[ind, 0], X[ind, 1], y[ind], depthshade = False, color='b')
